            ...
            <div className=" flex items-center  justify-between  border-b-[1px] border-b-gray-100 w-full px-5 py-3">
                ...
                <div className=" flex items-center">
                    {DocumentsBar?.map((item, index) => (
                        <div key={item.id} className={` ${selectedTab === item.id ? "flex items-center mr-5 bg-sirp-primaryLess2 p-1 rounded-lg cursor-pointer" : "flex items-center mr-5 p-1 cursor-pointer rounded-lg"}`} onClick={() => handleClick(item.id)}>
                            <span className="mr-2">
                                <Image src={require(`../../../assets/icons/${item.icon}`)} alt="" />
                            </span>
                            <span>{item.name}</span>
                        </div>
                    ))}
